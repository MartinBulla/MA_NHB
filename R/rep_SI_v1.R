...
#'
#' When referring to or reusing these materials, **please cite** the corresponding preprint [@bulla_MA_2026] and the associated repository [@bulla_supporting_2026], which contains all code, data, and outputs used in this replication.
...
#'
#' This project was initiated as part of the "Promoting reproduction and replication at scale" partnership between *Nature Human Behaviour* and the *Institute for Replication* [@editorial2024]. Accordingly, the Supporting information includes both **computational reproduction** (repeating the original analyses using the same dataset and code) and **robustness replication** (addressing the same research question using alternative analyses) of key findings from Ellis-Soto et al. [-@ellis-soto_historical_2023]. In recent terminology these correspond to computational and conclusive reproducibility, respectively [@Gomez2026].
#'
#' A complete report prepared for the *Nature Human Behaviour* - Institute for Replication initiative is available as a preprint [@bulla_replication_2025].

#'
#' ### Content of the repository
#' [Supporting information](https://martinbulla.github.io/MA_NHB/) is generated from the following repository structure:
#'
#' [original_paper/](https://github.com/MartinBulla/MA_NHB/tree/main/original_paper/) - Contains the original file structure provided by the authors, including `Data` and `Code` subfolders downloaded from the authors' [repository](https://doi.org/10.5281/zenodo.8052525). Due to copyright restrictions, these subfolders are not redistributed here and need to be obtained directly from the authors’ repository. The authors' article and its supplements are available at: https://www.nature.com/articles/s41562-023-01688-5
#'
#' [Data/](https://github.com/MartinBulla/MA_NHB/tree/main/Data/) - Stores additional datasets in subfolders:
...
#' - [rep_SI_v5.R](https://github.com/MartinBulla/MA_NHB/tree/main/R/rep_SI_v5.R) contains all code used to generate the paper and Supporting I=information outputs.
...
#' ## Research questions
#' The authors state that they investigated the following three questions:
...
#'
#' ## Results
#' The authors report that A-rated neighbourhoods have more than double the sampling density of D-rated neighbourhoods (1,592 vs. 712 records per km²; descriptive statistics). In a model of sampling density, the exponentiated coefficient for B-, C-, and D-graded neighbourhoods relative to A indicates multiplicative differences of 0.63, 0.37, and 0.26, respectively (authors' Table 1). Even the probability of being sampled at all differs: adjusted odds are 36% lower in D- than in A-rated neighbourhoods (adjusted odds ratio = 0.64; authors' Table 1).
#'
#' Survey completeness was also highest in A-graded neighbourhoods (~65%) and lowest in D-graded neighbourhoods (~59%; authors’ Table 1), implying greater uncertainty about species assemblages in areas of greatest historical disinvestment. Finally, the authors conclude that disparity is not shrinking with the rise of digital citizen science but is instead increasing. They report that the A:D ratio in cumulative sampling density rose from 2.021 in 2000 to 2.74 in 2020, corresponding to an increase of 35.6%, although it remains unclear whether these values represent descriptive statistics or model-based predictions.
#'
#' Based on these results, the authors argue  that "historically redlined neighbourhoods remain the most undersampled urban areas for bird biodiversity today, potentially impacting conservation priorities and propagating urban environmental inequities", and that this disparity in sampling “increased by 35.6% over the past 20 years” [@ellis-soto_historical_2023, abstract, p. 1869].
#'
#' ## Data
#' The authors obtained 12,297,506 georeferenced bird observations (class Aves) occurring within 9,851 HOLC-defined neighbourhoods across 202 cities. In the paper, however, 195 cities are reported, because cities sharing the same name but located in different states were inadvertently treated as the same city.
#'
#' Bird observations were retrieved from the [Global Biodiversity Information Facility](https://www.gbif.org/) and span records collected after HOLC categories were mapped (from 1933 to October 2022), with major contributions from the citizen-science platforms [eBird](https://ebird.org/home) and [iNaturalist](https://www.inaturalist.org/). Historical neighbourhood polygons were obtained from the [Mapping Inequality project](https://dsl.richmond.edu/panorama/redlining/).
#'
#' The authors defined three primary response variables: (1) whether a neighbourhood was sampled at all (yes/no), (2) neighbourhood sampling density (number of bird observations per km²), and (3) survey completeness (percentage of observed species richness relative to expected species richness).
#'
#' The authors also extracted several present-day covariates for each neighbourhood, including population density (per km²) derived by areal interpolation from the 2013–2017 American Community Survey [@goodchild1993], proportion of protected area [based on PAD-US, as a proxy for shared open space, @usgs2021], a composite Landsat Normalized Difference Vegetation Index (NDVI; 2014–2019) generated in [Google Earth Engine](https://earthengine.google.com/) (as a proxy for greenness), and mean annual precipitation and temperature for each city using monthly averages from 1979 to 2013 at 1-km² resolution [@karger2017].
#'
#' <hr style="margin: 1.5em 0 2em 0;">
#'
...
#'
#' However, the data deposited alongside the code were insufficient to execute the workflow as provided, preventing initial computational reproducibility. For a summary of provided items and computational reproducibility, see our Table S0.
#'
#+ tS0, out.width = "60%"
...
#' We were initially unable to run the provided Cleaning scripts because several required files were missing. Following a request from the Institute for Replication, the authors shared part of the missing data, specifically HOLC-grading files (made available via [Google Drive](https://drive.google.com/drive/u/0/folders/1ka97OdGV-sU3MTL4NUFM7_fDoElmTyEd)), and provided a link and instructions to download the mean annual precipitation and temperature data from the [CHELSA climatology platform](https://chelsa-climate.org/).
#'
#' Despite these additions, we were still unable to regenerate the prepared datasets. In addition to several minor issues, two major obstacles remained:
#' (a) the GBIF dataset reported in the paper (https://doi.org/10.15468/dd.ha9ksv) could not be accessed or recreated, as the DOI returned “DOI not found”; moreover, the Cleaning scripts require GBIF credentials that were not provided, and our own credentials failed authentication; and
#' (b) the CHELSA climate inputs were inconsistent with values used by the authors: site names could not be matched for most records, and temperature and precipitation values differed even for matched locations.
#'
#' Resolving these issues in the data-generation pipeline would have required substantial additional effort with uncertain outcomes. We therefore focused on computational reproducibility based on the deposited prepared data and the Analysis code provided by the authors.
#'
#'
#' ## Partial computational reproducibility from analysis code and prepared data
#' Identifying the scripts corresponding to individual figures, tables, and in-text results was not always straightforward, as scripts underlying such outputs were not consistently labelled. Nevertheless, after the authors provided additional missing files (`holc_ad_data.shp`, `NDVI_PAD_unique_ID.csv`), we resolved several minor code issues (see our Table S1 for details) and, using the deposited prepared datasets, successfully reproduced most main-text and supplementary outputs, including Fig. 2, Fig. 3, Fig. S1, and Tables S1–S3.
#'
#' ***
#'
#' <a name="T_S1">
...
#' ***
#'
#' We could not reproduce Fig. 1 nor the key claim of a 35.6% increase in disparity between grade A and D over 20 years. The authors did not describe how these outputs were generated, nor did we find code to do so.
#'
#' We reproduced Table S2 only partially. Specifically, we reproduced its first panel, which is mislabelled as log(sampling density) but non-transformed sampling density is shown. We could not reproduce the second panel because it was also labelled as log(sampling density). Note that according to the Table S2 description, three panels were expected: HOLC neighbourhood sampled, density of sampling and completeness of sampling, but it is unclear which sampling and completeness models should have been depicted.
#'
#' We also encountered difficulties reproducing the results reported in the authors’ Table S4. In the authors’ scripts, cumulative sampling density was not log-transformed, whereas the Table S4 description indicates a log transformation (here, log refers to the natural logarithm). Applying a log transformation to cumulative sampling density reproduced the values reported in Table S4. However, it remains unclear why a generalised additive model was used, as no smooth terms were fitted and the model effectively reduced to a simple Gaussian regression.
#'
#' We initially could not fully reproduce Table 1 because the best model was indicated to be shown, but the model behind the plot was not the best one according to AIC-selection found in the script. We initially had also difficulty replicating Fig. 4. because the output that resembles Fig. 4 is located in two scripts, each yielding different results:
#'
#' (a) According to the authors' [README](original_paper/Code/README_code.md), the code `05_paper_1_analyses_R4_check.Rmd` should contain all key analyses for the paper. We did find a script with the heading “6 trends”, which generates trend lines for all four HOLC categories, but instead of cumulative sampling density (authors' Fig. 4), it depicts the number of HOLC polygons that were sampled (our Fig. [S1](#F_S1) A). To complete the picture, we also plotted the trends for the number of sampling observations (our Fig. [S1](#F_S1) B).
#'
#' (b) We then found that the code `04_R4_uneven_biodiversity_data_2023.R` contains section *[7] Plot temporal trends 1933-2022 and 2000-2020*. The script was not initially running due to absolute folder assignments that were unintuitive regarding the location of the files. We searched for the required files (`R1_biodiv_col_code_by_holc_id_2000_2020.csv` and `R1_biodiv_trend_by_time_holc_id_1933_2022.csv`) among the folders provided by the authors and respecified the folder paths. With this code we were then able to reproduce Fig. 4.
#'
#' ***
#'
#+ F_S1, fig.width=.75*20*0.393701,fig.height=.75*8*0.393701
...
#' <a name="F_S1">
#' **Figure S1</a> | Change in # of sampled polygons (A) and # of observations (B) according to HOLC grade over time.** Each point represents the sum per year. Lines aid the visualisation by connecting the points. Colour indicates HOLC grade category.
#'
#' <hr style="margin: 1.5em 0 2em 0;">
#'
...
#'
#' Models (5) and (6) possibly represent multiple models where each predictor (NDVI, protected area (%), population density (per km²), and perhaps mean temperature and precipitation) was added separately or sequentially one by one.
#'
...
#'
#' **Second**, the authors compared mixed-effect models within each set (i.e. for each response) and selected the best one using Akaike information criterion (AIC) [@Akaike2025], a method that is the most robust only if the compared mixed-effect models have the same random effect structure [@verbyla2019]. However, the authors compared models differing in both fixed and random effects, a practice advised against [@zuur2009; @greven2010; @vaida2005].
#'
#' A more standard approach would have been to fit a single model controlling for all discussed variables and for non-independence of data points. Their data with n around 9,000 neighbourhoods and clear data structure allows that. If in doubt, fitting a simpler model and comparing its estimates with the complex one would suffice. Such practice reduces the risk of reporting spurious effects [@forstmeier2011; @whittingham2006].
#'
#' **Finally**, the authors do not describe in their Methods, nor provide in their scripts, any checks of model assumptions. This is problematic, as some of their models show diagnostic issues, including limited control for non-independence of data points (spatial and/or temporal) and heteroscedasticity, and do not adequately capture key features of the data (see [Model assumptions section](#Model)) [@zuur2010; @ives2021; @knief2021].
#'
#' **Here**, we respecified the model set for each response, checked the model assumptions (and if needed respecified the model/s) and compared the model estimates. We include plausible models from the authors' set and included further two models either with HOLC grade as a single fixed effect (simple models) or including also all other continuous control variables (discussed by the authors) as fixed effects (NDVI, protected area (%), population density (per km²), mean temperature and mean precipitation; full model). For our simple and full models, we varied the random structure (i) to mirror the authors' logic, but using only discussed and meaningful variables (e.g. excluding random slopes with the same number of levels as the corresponding random intercept), and to show whether estimates and their uncertainty[@Dushoff2019] change if we (ii) account for non-independence of data points, and (iii) if needed model the heteroscedasticity.
#'
...
#' (6) ~ HOLC grade + NDVI + protected area % + population density + (1 + HOLC grade | metropolitan area)
...
#' ***
#'
#+ F_S2, fig.width = 25/2.5, fig.height = 8/2.5
  ...
  # model ass
  if(recompute_diag){
  m_ass(name = 'Fig_S2a', mo = samp_d_binary_holc, dat = h,
  ...
  m_ass(name = 'Fig_S2b', mo = m0, dat = h,
        categ = 'holc_grade', show_binned = TRUE, PNG = TRUE)

  m_ass(name = 'Fig_S2c', mo = samp_d_binary_holc_rirs, dat = h,
        categ = 'holc_grade', show_binned = TRUE, PNG = TRUE)

  m_ass(name = 'Fig_S2d', mo = m1, dat = h,
        categ = 'holc_grade', show_binned = TRUE, PNG = TRUE)

  m_ass(name = 'Fig_S2e', mo = m1b, dat = h,
  ...
  m_ass(name = 'Fig_S2f', mo = m2, dat = h,
        categ = 'holc_grade', cont = 'area_holc_km2', trans = 'log', show_binned = TRUE, PNG = TRUE)

  m_ass(name = 'Fig_S2g', mo = m1p, dat = h,
  ...
  m_ass(name = 'Fig_S2h', mo = m2p, dat = h,
        categ = 'holc_grade',
  ...
  m_ass(name = 'Fig_S2i', mo = m3p, dat = h,
        categ = 'holc_grade',
        cont = c('area_holc_km2','ndvi','pct_pa','pop_per_km','mean_temp_c','mean_precip_mm'),
        trans = c('log','none','none','none','none','none'),
        show_binned = TRUE, PNG = TRUE)
  ...
  # model ass
  if(recompute_diag){
  m_ass(name = 'gaus_Fig_S2a', mo = samp_m0_g, dat = h,
      categ = 'holc_grade',  PNG = TRUE)

  m_ass(name = 'gaus_Fig_S2b', mo = m0_g, dat = h,
        categ = 'holc_grade',  PNG = TRUE)

  m_ass(name = 'gaus_Fig_S2c', mo = samp_m1_g, dat = h,
        categ = 'holc_grade',  PNG = TRUE)

  m_ass(name = 'gaus_Fig_S2d', mo = m1_g, dat = h,
        categ = 'holc_grade', PNG = TRUE)

  m_ass(name = 'gaus_Fig_S2e', mo = m1b_g, dat = h,
        categ = 'holc_grade', cont = 'area_holc_km2', trans = 'log',
        PNG = TRUE)

  m_ass(name = 'gaus_Fig_S2f', mo = m2_g, dat = h,
        categ = 'holc_grade', cont = 'area_holc_km2', trans = 'log',
        PNG = TRUE)

  m_ass(name = 'gaus_Fig_S2g', mo = m1p_g, dat = h,
  ...
  m_ass(name = 'gaus_Fig_S2h', mo = m2p_g, dat = h,
        categ = 'holc_grade',
        cont = c('area_holc_km2','ndvi','pct_pa','pop_per_km','mean_temp_c','mean_precip_mm'),
        trans = c('log','none','none','none','none','none'),
        PNG = TRUE)

  m_ass(name = 'gaus_Fig_S2i', mo = m3p_g, dat = h,
        categ = 'holc_grade',
        cont = c('area_holc_km2','ndvi','pct_pa','pop_per_km','mean_temp_c','mean_precip_mm'),
        trans = c('log','none','none','none','none','none'),
      PNG = TRUE)
...
#' ***
#'
#+ F_S3, fig.width = 25/2.5, fig.height = 12/2.5
  ...
  # non zero data
    ...
    # model ass
    if(recompute_diag){
      m_ass(name = 'Fig_S3a', mo = d_ri, dat = hB_,
            categ = 'holc_grade',  PNG = TRUE)

      m_ass(name = 'Fig_S3b', mo = mB0, dat = hB_,
            categ = 'holc_grade',  PNG = TRUE)

      m_ass(name = 'Fig_S3c', mo = d_rirs, dat = hB_,
            categ = 'holc_grade',  PNG = TRUE)

      m_ass(name = 'Fig_S3d', mo = mB1, dat = hB_,
            categ = 'holc_grade', PNG = TRUE)

      m_ass(name = 'Fig_S3e', mo = mB2, dat = hB_,
            categ = 'holc_grade', PNG = TRUE)

      m_ass(name = 'Fig_S3f', mo = d_fe_rirs, dat = hB_,
            categ = 'holc_grade',
            cont = c('ndvi','pct_pa','pop_per_km'),
            PNG = TRUE)

      m_ass(name = 'Fig_S3g', mo = mB1p, dat = hB_,
            categ = 'holc_grade',
            cont = c('ndvi','pct_pa','pop_per_km','mean_temp_c','mean_precip_mm'),
            PNG = TRUE)

      m_ass(name = 'Fig_S3h', mo = mB2p, dat = hB_,
            categ = 'holc_grade', cont = c('ndvi','pct_pa','pop_per_km','mean_temp_c','mean_precip_mm'),
            PNG = TRUE)

      m_ass(name = 'Fig_S3i', mo = mB3p, dat = hB_,
            categ = 'holc_grade',
            cont = c('ndvi','pct_pa','pop_per_km','mean_temp_c','mean_precip_mm'),
            PNG = TRUE)
    }
  ...
  # including zero data (with a small offset to allow log)
    ...
    # model ass
    if(recompute_diag){
      m_ass(name = 'a_Fig_S3a', mo = d_ri_a, dat = hB,
            categ = 'holc_grade',  PNG = TRUE)

      m_ass(name = 'a_Fig_S3b', mo = mB0_a, dat = hB,
            categ = 'holc_grade',  PNG = TRUE)

      m_ass(name = 'a_Fig_S3c', mo = d_rirs_a, dat = hB,
            categ = 'holc_grade',  PNG = TRUE)

      m_ass(name = 'a_Fig_S3d', mo = mB1_a, dat = hB,
            categ = 'holc_grade', PNG = TRUE)

      m_ass(name = 'a_Fig_S3e', mo = mB2_a, dat = hB,
            categ = 'holc_grade', PNG = TRUE)

      m_ass(name = 'a_Fig_S3f', mo = d_fe_rirs_a, dat = hB,
            categ = 'holc_grade',
            cont = c('ndvi','pct_pa','pop_per_km'),
            PNG = TRUE)

      m_ass(name = 'a_Fig_S3g', mo = mB1p_a, dat = hB,
            categ = 'holc_grade',
            cont = c('ndvi','pct_pa','pop_per_km','mean_temp_c','mean_precip_mm'),
            PNG = TRUE)

      m_ass(name = 'a_Fig_S3h', mo = mB2p_a, dat = hB,
            categ = 'holc_grade', cont = c('ndvi','pct_pa','pop_per_km','mean_temp_c','mean_precip_mm'), offset = TRUE,
            PNG = TRUE)

      m_ass(name = 'a_Fig_S3i', mo = mB3p_a, dat = hB,
            categ = 'holc_grade',
            cont = c('ndvi','pct_pa','pop_per_km','mean_temp_c','mean_precip_mm'),
            PNG = TRUE)
    }
  ...
  # RATES
    ...
    # model ass
    if(recompute_diag){
      m_ass(name = 'r_Fig_S3a', mo = d_ri_r, dat = hB,
        categ = 'holc_grade', cont = 'area_holc_km2', offset = TRUE, show_binned = TRUE,
        PNG = TRUE)

      m_ass(name = 'r_Fig_S3b', mo = mB0_r, dat = hB,
            categ = 'holc_grade', cont = 'area_holc_km2', offset = TRUE, show_binned = TRUE, PNG = TRUE)

      m_ass(name = 'r_Fig_S3c', mo = d_rirs_r, dat = hB,
            categ = 'holc_grade', cont = 'area_holc_km2', offset = TRUE, show_binned = TRUE, PNG = TRUE)

      m_ass(name = 'r_Fig_S3d', mo = mB1_r, dat = hB,
            categ = 'holc_grade', cont = 'area_holc_km2', offset = TRUE, show_binned = TRUE,  PNG = TRUE)

      m_ass(name = 'r_Fig_S3e', mo = mB2_r, dat = hB,
            categ = 'holc_grade', cont = 'area_holc_km2', offset = TRUE, show_binned = TRUE, PNG = TRUE)

      m_ass(name = 'r_Fig_S3f', mo = d_fe_rirs_r, dat = hB,
            categ = 'holc_grade',
            cont = c('ndvi','pct_pa','pop_per_km'),
            offset = TRUE, show_binned = TRUE, PNG = TRUE)

      m_ass(name = 'r_Fig_S3g', mo = mB1p_r, dat = hB,
            categ = 'holc_grade',
            cont = c('ndvi','pct_pa','pop_per_km','mean_temp_c','mean_precip_mm','area_holc_km2'),
            offset = TRUE, show_binned = TRUE, PNG = TRUE)

      m_ass(name = 'r_Fig_S3h', mo = mB2p_r, dat = hB,
            categ = 'holc_grade', cont = c('ndvi','pct_pa','pop_per_km','mean_temp_c','mean_precip_mm','area_holc_km2'),
            offset = TRUE, show_binned = TRUE, PNG = TRUE)

      m_ass(name = 'r_Fig_S3i', mo = mB3p_r, dat = hB,
            categ = 'holc_grade',
            cont = c('ndvi','pct_pa','pop_per_km','mean_temp_c','mean_precip_mm','area_holc_km2'),
            offset = TRUE, show_binned = TRUE, PNG = TRUE)
    }
...
#' ***
#'
#+ F_S4, fig.width = 25/2.5, fig.height = 4/2.5
...
# model ass
if(recompute_diag){
m_ass(name = 'Fig_S4a', mo = c_ri, dat = hC,
      categ = 'holc_grade',
      PNG = TRUE)

m_ass(name = 'Fig_S4b', mo = mC0, dat = hC,
      categ = 'holc_grade', PNG = TRUE)

m_ass(name = 'Fig_S4c', mo = c_rirs, dat = hC,
      categ = 'holc_grade', cont = 'area_holc_km2', PNG = TRUE)

m_ass(name = 'Fig_S4d', mo = mC1, dat = hC,
      categ = 'holc_grade', cont = 'area_holc_km2', PNG = TRUE)

m_ass(name = 'Fig_S4e', mo = mC2, dat = hC,
      categ = 'holc_grade', cont = 'area_holc_km2',PNG = TRUE)

m_ass(name = 'Fig_S4f', mo = c_fe_rirs, dat = hC,
      categ = 'holc_grade',
      cont = c('ndvi','pct_pa','pop_per_km'),
      PNG = TRUE)

m_ass(name = 'Fig_S4g', mo = mC1p, dat = hC,
      categ = 'holc_grade',
      cont = c('ndvi','pct_pa','pop_per_km','mean_temp_c','mean_precip_mm','area_holc_km2'),
      PNG = TRUE)

m_ass(name = 'Fig_S4h', mo = mC2p, dat = hC,
      categ = 'holc_grade', cont = c('ndvi','pct_pa','pop_per_km','mean_temp_c','mean_precip_mm','area_holc_km2'), PNG = TRUE)

m_ass(name = 'Fig_S4i', mo = mC3p, dat = hC,
      categ = 'holc_grade',
      cont = c('ndvi','pct_pa','pop_per_km','mean_temp_c','mean_precip_mm','area_holc_km2'),
      PNG = TRUE)
}
...
#' <a name="F_S4">
#' **Figure S4</a> | Differences in estimated sampling completeness between HOLC grades across modelling approaches.** Points represent fixed-effect contrasts for HOLC grades B-D (columns), relative to HOLC grade A, in completeness of sampling (index), while horizontal lines are 95% confidence intervals. The vertical dashed line indicates zero difference. Red open circles indicate models specified by the original authors; blue filled circles those specified by us. The y-axis lists model structure: terms outside parentheses are fixed effects (`*` denotes interactions), terms inside parentheses denote random effects (variables left of `|` are random slopes, variables right of `|` are random intercepts). All models are based on n = `r nrow(hC)` HOLC polygons (neighbourhoods).
#'
#' ***
#'
#'
#' Our replication supports the same qualitative trends but quantifies a larger A-D disparity, and demonstrates that robustness improves with model complexity and adequate random-effect specification (Fig. [S4](#F_S4)).
...
#'
#' (i) Visualisation of temporal trends for each HOLC grade (Fig. 4).
#' (ii) Claim about 35.6% increase in relative disparity between HOLC grade A and D from 2000 to 2020.
...
#'
#' ### i. Visualisation of temporal trends
#'
#' The authors' Fig. 4 visualises temporal trends in cumulative sampling density for each HOLC grade. Cumulative metrics are not ideal for representing temporal dynamics because they mechanically increase over time and are sensitive to inter-annual variation in sampling effort and extreme high-density events (‘hotspots’), which can inflate totals and obscure underlying patterns. In cumulative space, differences reflect both current sampling rates and historical accumulation, making it difficult to attribute divergence to contemporary processes.
#'
#' To isolate temporal variation from cumulative effects, one can instead visualise annual sampling density (total observations per year divided by total HOLC-grade area; our Fig. [S5](#F_S5)). This removes the mechanical increase inherent to cumulative metrics (as in authors' Fig. 4), but still aggregates across polygons and therefore masks spatial heterogeneity. We therefore treat this as a descriptive baseline rather than a preferred analytical approach (see next sections).
#'
#' ***
#'
#+ F_S5, fig.width=9/2.5,fig.height=7/2.5

ggplot(tt00, aes(x = year, y = sampling_density, col = holc_grade)) +
  stat_smooth(aes(color = holc_grade), size = 1, show.legend = FALSE, alpha = 0.3) +
  geom_point() +
  #geom_line() +
  scale_color_manual(values = holc_pal, name = 'HOLC grade') +
  labs(x = "Year", y = "Sampling density\n[bird observations per km²]") +
  theme_light() +
  theme(
        legend.position=c(.2,.8), # position legend inside plot
...
#' <a name="F_S5">
#' **Figure S5</a> | Change in overall sampling density between HOLC grades over time.** Each point represents sampling density per HOLC grade and year (sum of all observations divided by the total area for the given grade). Lines represent local regression non-parametric smoothing and shaded areas its 95% confidence intervals. Note that the visualisation highlights (see also our Fig. [S6](#F_S6)) that (i) sampling density was similar across all categories until ~2010, (ii) then differences increased, (iii) subsequently levelled off, and that (iv) 2020 may represent an atypical year.
#'
#' ***
#'
#'
#+ F_S6, fig.width=21*0.393701,fig.height=14*0.393701
...
#' <a name="F_S6">
#' **Figure S6</a> | Change in sampling density between HOLC grades over time.** Each point represents sampling density per HOLC grade and year (sum of all observations per year and grade divided by the total area for the given grade) with the year indicated for some observations. Red lines with shaded area represent local regression non-parametric smoothing with its 95% confidence intervals. Dotted line indicates unity, i.e. point on the line highlights the same sampling density, above the line higher sampling for y and below the line higher sampling for x.
#'
#' <hr style="margin: 1.5em 0 2em 0;">
#'
#' ### ii. Quantification of disparity
#'
#' We did not find the code generating the 35.6% claim (Abstract p. 1869, Results p. 1871), and therefore approximated the calculation. We suspect that the authors used the yearly sampling density (total observations per year divided by total HOLC-grade area; data behind our Fig. [S5](#F_S5)) as that is the only temporal data we found.
#'
#' Using the yearly sampling density metric and calculating the ratio between grade A and D (A/D) for 2000 and for 2020, we found a `r dispar`%  difference between the two years, which contrasts with the authors' 35.6%. Our result closely matches the value (~40%) reported in the authors' Fig. 4 legend. Importantly, plotting A-D disparity over time reveals non-linear temporal trends, with zero or even opposite disparities (our Fig. [1](#F_1)). Moreover, the choice of metric matters because different metrics reflect different underlying sampling mechanisms.
#'
#' Disparity between HOLC grades A and D can be decomposed into two mechanisms: (1) the probability that a neighbourhood is sampled at all (“coverage”), and (2) the intensity of sampling within sampled neighbourhoods. Metrics based on means or totals (especially arithmetic mean sampling density and total observations per HOLC-grade area) combine these components and are therefore sensitive to extreme high-density events (‘hotspots’) and to city-specific variation. Geometric-mean sampling density isolates the typical neighbourhoods-year by down-weighing extreme values, whereas arithmetic means emphasise rare high-density neighbourhoods. Trimming extreme hotspots (top 0.5% of neighbourhoods) visualises these issues (our Fig. [1](#F_1)).
#'
#' Across 2000–2020, alternative metrics that represent different sampling mechanisms reveal a different picture from the original 35.6% claim (our Fig. [1](#F_1)). The proportion of neighbourhoods sampled shows a long-term decline in A–D disparity, while intensity among sampled neighbourhoods increases only modestly before stabilising after ~2015, far less than the increases implied by hotspot-sensitive metrics. In contrast, hotspot-sensitive metrics produce the largest apparent rise. Note that some heavily sampled neighbourhoods lack extensive green spaces or contain museums or universities; thus, ideally their validity should be verified, which was beyond the scope of our replication. After trimming extreme hotspots, the pronounced increase in mean-based disparities largely disappears. This demonstrates that the originally reported 35.6% rise in disparity is not a stable temporal signal but appears to be driven by a metric disproportionately influenced by a small subset of heavily sampled neighbourhoods (primarily after 2010) and by inter-annual sampling peaks, features that inflate total-area averages and occur irrespective of HOLC grade. Importantly, the raw sampling densities of A and D neighbourhoods were nearly indistinguishable until about 2010 (our Fig. [1](#F_1)).
#'
#' These results question the use of arbitrary, two-point comparisons (2000 vs 2020). Including pre-2000 years reveals an even more complex temporal picture, although these early data are likely less reliable because online platforms were not yet in use and earlier observations may have been added retrospectively.
#'
#' ***
#'
...
#' <a name="F_1">
#' **Figure 1</a> | Change in relative disparity in bird sampling density between HOLC grade A and D over time.** Each point represents relative percentage difference (two left columns), absolute difference (two middle columns) in sampling density of A given D (with D being a baseline) based on overall sampling density (i.e. sum of all A or D observation divided by the total area of A or D; **first row**), mean sampling density per HOLC grade and year (**second row**), proportion of sampled neighbourhoods (**third row**), mean sampling density across sampled neighbourhoods (i.e. excluding non-sampled ones; **fourth row**) and geometric mean in sampling density (**fifth row**). The right column shows the actual values for A and D HOLC grades. Dots represent yearly values (for all data: large dots; for data with top 0.5% observations trimmed: small dots). Lines in the first four columns represent local regression non-parametric smoothing and shaded areas 95% confidence intervals. Colour in the right column indicates all data (black) or data with top 0.5% trimmed (red), in the left column the HOLC grade category (A in green, D in red). The **top row** represents the aggregation likely used by the authors to support their claim about 35.6% increase, the **other rows** the aggregation done by us. Note that across 2000–2020, alternative metrics (rows 2-5), which represent different sampling mechanisms and down-weight extreme hotspots or isolate distinct sampling processes, reveal a markedly different temporal pattern from the original 35.6% claim (first row).
#'
#' ***
#'
#' Importantly, the HOLC grade trends are not only volatile across methods, but also across cities (our Extended Data Fig. [1](#F_E1) and [2](#F_E2)). To illustrate typical within-city sampling patterns, we classified cities as *A-skewed*, *D-skewed*, and *mixed* using the following data-driven procedure. The goal was to identify cities with clear and stable differences between HOLC grades while avoiding idiosyncratic cases dominated by single-year hotspots. For each city and year we computed the geometric mean sampling density for HOLC grades A and D separately. The geometric mean was used because it reflects the typical sampling density of neighbourhoods in a given year while down-weighting single-year outliers. We then calculated a yearly, city-specific A/D ratio and summarised it using the city-level median. We combined this median with a measure of temporal consistency to classify cities:
...
#' <a name="F_E1">
#' **Extended Data Figure 1</a> | Change in HOLC grade sampling density per km² over time across cities.** Lines represent locally estimated scatterplot smoothing or predictions from generalised additive models (generated by the *stat_smooth* function from the *ggplot2* R-package). Line colour indicates the HOLC grade.
#'
#' ***
#'
#+ F_E2, fig.width = .9*20/2.5, fig.height = .9*12.5/2.5
...
#' <hr style="margin: 1.5em 0 2em 0;">
#'
#' ### iii. Formal analyses of temporal trends

#' To formally investigate temporal differences among HOLC grades, the authors modelled yearly sampling density per HOLC grade (log-transformed) using a generalised linear model, effectively assuming a linear temporal trend without allowing for non-liner temporal structure. However, modelling sampling density as a response variable collapses total area, sampling coverage (whether a neighbourhood is sampled at all), and sampling intensity into a single quantity within sampled neighbourhoods. In particular, variation in total area across HOLC grades is embedded in the response rather than explicitly accounted for.
#'
#' We thus re-analysed the authors’ year-by-HOLC-grade aggregates using a negative binomial generalised linear model of counts, including log(area(km²)) as an offset (our Table [S3](#T_S3), Fig. [S7](#F_S7)). This approach models sampling rates per unit area while accounting for overdispersion and separating the effect of sampling effort from differences in available area.
#'
#' ***
#'
#' <a name="T_S3">
#' **Table S3 | Change in sampling density across time in relations to HOLC grade based on negative binomial model with offset**</a>
...
#' ***
#'
#' Note that when we restrict the dataset to the post-2010 period, i.e. when absolute sampling density differences are no longer close to zero, coinciding with the rise of smartphone-driven community science, the difference in temporal trends (slopes) between HOLC grades A and D is no longer statistically distinguishable (our Table [S3](S3)).
#'
#' Moreover, the above models use aggregates per year and HOLC grade (i.e. total observations per HOLC grade divided by the total area of that grade). As discussed above, such aggregation is sensitive to the chosen aggregation metric, can be driven by a small number of high-density observations, and can thus yield volatile results (our Fig. [1](#F_1)). In addition, aggregations limit the ability to account for spatial and temporal non-independence.
#'
#' To address these limitations, we return to the raw data, which allows modelling observations at the neighbourhood-year level and explicitly accounting for spatial and temporal structure, as well as potential confounding variables.
#'
#' We thus constructed a dataset containing the number of observations for each unique neighbourhood in each year (i.e. city-specific HOLC neighbourhoods with unique identifiers; n = `r comma(nrow(d))` neighbourhood-year records; see Extended Data Fig. [1](#F_E1) for the data distribution).
#'
#' We then fitted negative binomial mixed-effects models with number of observations as the response, log-transformed neighbourhood area (km²) as an offset, and year (continuous) in interaction with HOLC grade (four-level factor) as fixed effects, while accounting for non-independence of data points via random effects. This formulation models counts directly while standardising for area, rather than analysing derived density measures.
#'
#' We specified five models differing in their random-effects structure and compared their fixed-effect estimates:
#'
#' (1) Random intercepts for state, city nested within state, and neighbourhood.
#' (2) Explicitly nested random intercepts for state, city, HOLC grade, and neighbourhood.
#' (3) As in (1), with a random slope of year within city.
#' (4) As in (1), with a random slope of year within neighbourhood.
#' (5) Random intercept for neighbourhood, and random slope of year within state, city, and HOLC grade, i.e. based on the nested structure in (2).
#'
#' For model (5), a ranodom slope of year within the fully specified nested random-intercept structure (version (2)) was non-identifiable, likely due to overparameterisation. We thus simplified the random-effects structure to levels where slope variance was identifiable and did not bias fixed-effect estimates.
#'
#' Moreover, to accommodate slight heteroskedasticity with respect to HOLC grade and city area, we also fitted models with dispersion modelled as a function of these predictors (`dispformula = ~ holc_grade_D + log(area_holc_km2)`). These yielded nearly identical results as the models without dispersion component, and we hence report the simpler models.
#' <br><br>
#'
#' #### A. Contrasts
#'
...
#' <a name="F_S8">
#' **Figure S8</a> | Estimates of log sampling density between HOLC grades over time.** Dots for intercepts give the log sampling density (observations per km² per year) for each HOLC grade at the mean year; dots for slopes give the change in log sampling density per 1 standard deviation increase in year. Horizontal lines are 95% Wald confidence intervals. Colour indicates random-effects structure (variables left of `|` are random slopes, right of `|` random intercepts, and '/' indicates nesting). The depicted estimates represent per grade estimates (not contrasts, which are shown in Extended Data Fig. [3](#F_E3)). The top row contains estimates for a dataset spanning 2000-2020 (n = `r comma(nrow(tt00))` sum of observations per grade and year for the linear model, and n = `r comma(nrow(d00))` sum of observations per polygon and year); the bottom row contains estimates for a dataset from 2010-2020 (n = `r comma(nrow(tt10))` and n = `r comma(nrow(d10))`, respectively).
#'
#' <hr style="margin: 1.5em 0 2em 0;">
#'
#' #### C. Smooth models
#'
#' The above models are intended as appropriate alternatives to the authors' models that, in our view, do not fully account for the data structure. However, the authors' models impose linear trends that do not capture the non-linear structure visible in the data (our Fig. [1](#F_1), Extended Data Fig. [1](#F_E1)), as well as differences in 2000-2020 vs 2010-2020 estimates (our Extended Data Fig. [3](#F_E3), and Fig. [S8](#F_S8)). To capture these fluctuations, we analysed temporal changes in observation rates using a generalised additive mixed model (GAM) fitted with `bam()` in `mgcv`. The response was the annual number of bird observations per HOLC polygon, modelled with a negative-binomial distribution and a `log(area)` offset (centred) to estimate rates per km². Year (scaled) was fitted with a penalised regression spline, with grade-specific deviations (`s(year, by=holc_grade)`), allowing flexible non-linear trends. Random intercepts for state and city, and city-specific random slopes of year (`bs="re"`) accounted for spatial hierarchy and differing temporal trajectories among cities, which capture city-level departures from the global smooth but avoids over-parameterisation in rarely sampled city–grade combinations. Models were fitted via REML using discrete smooths to support large datasets (~200k rows).
#'
#' **Model-based predictions and contrasts.** To visualise temporal trends in sampling density for each HOLC grade, we obtained population-level predictions from the negative-binomial GAM by marginalising over all random effects. Predictions were generated from the model’s linear predictor matrix (`type = "lpmatrix"`) while excluding random-effect smooths (`s(state)`, `s(city_state)`, `s(neighbourhood)`, and `s(city_state):year_s)`. This yields expected values for an “average” neighbourhood (i.e. with mean-zero random effects).
#'
#' To quantify differences between grades, we constructed linear contrasts of the prediction matrices (e.g. A − D) and computed associated Wald intervals via the delta method (using the full model variance–covariance matrix). Predictions were transformed back to the original response scale (observations per km²), enabling interpretation on (i) the model (link) scale, (ii) the percentage scale, and (iii) the absolute scale.
#'
#' ***
#'
...
#'
#' #### Conclusions
#' These non-linear patterns contradict the authors’ reported 35.6% linear increase. Our estimates show that (i) A-D disparity does not follow a linear trajectory, (ii) the relative magnitude of change exceeds ~200% across the 2000-2020 period, i.e. is substantially greater than 35.6%, and (iii) absolute differences remain small (typically <25 observations per km²), indicating that the apparent inflation of disparity is driven largely by relative scaling rather than large absolute differences.
#'
#' <hr style="margin: 1.5em 0 2em 0;">
#'
#' # Model assumptions {#Model_ass}

#' The model formula consists of the model type (lm - linear model, glm - generalised linear model, lmer - linear mixed-effects model, glmer - generalised linear mixed-effects model, glmmTMB – generalised linear mixed‐effects model fitted via Template Model Builder, gam - generalised additive model, glm.nb – negative binomial generalised linear model, bam – generalised additive model for large datasets), the response (left of ~), predictors (right of ~) with predictors in parentheses indicating random effects, predictors without parentheses indicate fixed effects. In mixed-effects notation, terms to the right of '|' define grouping variables (random intercepts by default), while terms to the left specify the associated random effects (e.g. slopes). Explicit nesting is indicated by '/' (e.g. state/city). 'scale' indicates z-transformation, whereas `log()`` (in R) denotes the natural logarithm.
#'
#' ## Fig. 2
...
#'
#' ## Extended Data Fig. 3 and Fig. S8
...
#'
#' ## Fig S2 - Sampled yes or no
#' Below diagnostics of binomial models (Pearson residuals, binned fits, QQ-plots, partial autocorrelation, and spatial semivariogram) indicate good fit and no substantial temporal or spatial autocorrelation (Moran’s I ≈ 0). Pearson dispersion ≈ 0.6 suggested mild underdispersion, which makes inference conservative rather than inflated, which is desired. Gaussian mixed models, despite non-normal residuals, yielded consistent effect directions and magnitudes compared to binomial models (see Fig. [S2](#F_S2)), supporting the robustness of the main results (cf. Knief & Forstmeier 2021).
#'
#' ### binomial
rm(list=setdiff(ls(), c("holc_pal"))); invisible(gc())

knitr::include_graphics(here::here("Output", "Model_ass", "Fig_S2a.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "Fig_S2b.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "Fig_S2c.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "Fig_S2d.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "Fig_S2e.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "Fig_S2f.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "Fig_S2g.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "Fig_S2h.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "Fig_S2i.png"))
#'
#' ### gaussian
knitr::include_graphics(here::here("Output", "Model_ass", "gaus_Fig_S2a.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "gaus_Fig_S2b.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "gaus_Fig_S2c.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "gaus_Fig_S2d.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "gaus_Fig_S2e.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "gaus_Fig_S2f.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "gaus_Fig_S2g.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "gaus_Fig_S2h.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "gaus_Fig_S2i.png"))

#' ## Fig. S3 - Sampling density
...
knitr::include_graphics(here::here("Output", "Model_ass", "Fig_S3a.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "Fig_S3b.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "Fig_S3c.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "Fig_S3d.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "Fig_S3e.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "Fig_S3f.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "Fig_S3g.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "Fig_S3h.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "Fig_S3i.png"))
#'
...
knitr::include_graphics(here::here("Output", "Model_ass", "a_Fig_S3a.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "a_Fig_S3b.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "a_Fig_S3c.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "a_Fig_S3d.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "a_Fig_S3e.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "a_Fig_S3f.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "a_Fig_S3g.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "a_Fig_S3h.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "a_Fig_S3i.png"))
#'
#' ### Negative-binomial count models with `ln(area km²)` offset
knitr::include_graphics(here::here("Output", "Model_ass", "r_Fig_S3a.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "r_Fig_S3b.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "r_Fig_S3c.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "r_Fig_S3d.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "r_Fig_S3e.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "r_Fig_S3f.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "r_Fig_S3g.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "r_Fig_S3h.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "r_Fig_S3i.png"))
#'

#' ## Fig. S4 - Completeness of sampling
...
#' ## Table S3 & Fig. S7
knitr::include_graphics(here::here("Output", "Model_ass", "Table_S3_all_years.png"))
knitr::include_graphics(here::here("Output", "Model_ass", "Table_S3_2010-2020_years.png"))
...
#'
#' <a name="T_S4">
#' **Table S4 | System session info.** </a>
...
#'
#' <a name="T_S5">
#' **Table S5 | Info about used packages.** </a>
