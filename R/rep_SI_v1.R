...
#' 
#' This project was initiated as a part of the "Promoting reproduction and replication at scale" partnership between *Nature Human Behaviour* and the *Institute for Replication* [@editorial2024]. Accordingly, the Supporting Information includes both **computational reproduction** (repeating the original analyses using the same dataset and code) and **robustness replication** (addressing the same research question using alternative analyses) of key findings from Ellis-Soto et al. [-@ellis-soto_historical_2023]. 
#' 
...
#' Identifying the scripts corresponding to individual figures, tables, and in-text results was not always straightforward, as scripts underlying such outputs were not consistently labelled. Nevertheless, after the authors provided additional missing files (`holc_ad_data.shp`, `NDVI_PAD_unique_ID.csv`), we resolved several minor code issues (see Table S1 for details) and, using the deposited prepared datasets, successfully reproduced most main-text and supplementary outputs, including Fig. 2, Fig. 3, Fig. S1, and Tables S1–S3. 
#' 
#' ***
#' 
#' <a name="T_S1">
...
#' ***
#' 
#' We initially could not fully reproduce: 
#' (i) Table 1 because the authors indicate that the AIC-best model is reported, yet the model underlying Table 1 is not the AIC-best model identified in their script, and  
#' (ii) Table S4 because the model description did not match the script and the underlying data were accidentally multiplied (our Table [S3](#T_S3a); see ‘d’ for output corresponding to authors’ Table S4).  
#' (iii) The data multiplication also obscured the reproducibility also of Fig. 4 (see next section). 
#' 
#' ## Coding error
#' The major coding error that we found in the authors' script preparing temporal trend data (`04_R4_uneven_biodiversity_data_2023.R`, section *[7] Plot temporal trends 1933-2022 and 2000-2020*) unintentionally multiplied the dataset (our Fig. [S1](#F_S1)). Hence, Fig. 4 and Table S4 did not depict the actual data (see our Fig. [S2](#F_S2) and Table [S3](#T_S3a)), and the analytical basis of the reported 35.6% claim cannot be traced or verified from the provided code and data.  
#' 
...
#' ***
#' 
#' The corrected figures we generated show that (i) sampling across all categories was similar until ~2010. (ii) Then differences in sampling between categories increased, and (iii) subsequently leveled off. These differences are more readily apparent when sampling densities of two categories are plotted against each other (our Fig. [S3](#F_S3)) or when relative and absolute difference are examines (see section *Temporal trends* below). Notabkt, the patterns (our Fig. [S3](#F_S3)) are sensitive to the used aggregation metric.
#' 
#' ***
#'  
...
#' ***
#'   
#' These figures highlight patterns discussed by the authors. Specificially, they suggest that 2020 may represent an atypical year (e.g. due to COVID-19-related increase in human presence in urban green spaces), that an increase in disparity occured around 2010 (coinciding with the introduction of smartphones), and that disparity levelled off thereafer. However, these patterns depend on the aggregation metric used (our Fig. [4](#F_4)) and, as in the authors' Fig. 4, the aggregations do not account for spatial and temporal non-independence of data points or for potential confounding factors. 
#' 
#' We also encountered difficulties reproducing the results reported in the authors’ Table S4. In the authors’ scripts, sampling density was not log-transformed, whereas the Table S4 description indicates a log transformation (here, log refers to the natural logarithm). Applying a log transformation to sampling density and using the multiplied dataset reproduced the values reported in Table S4 (see our Table S3). However, it remains unclear why a generalised additive model was used, as no smooth terms were fitted and the model effectively reduced to a simple Gaussian regression.
#' 
#' <br>
#'    
#' <a name="T_S3a"> 
...
tab_model(m_density_mul_ln, m_density_ok_ln,  auto.label = T, string.ci='95%CI', title = "Generalized linear model on log(sampling density)", dv.labels = c("Multiplied # of observations", "Actual # of observations"))
#' ***
#' 
#' <br>
#'  
...
tab_model(gam_density_mul_ln, gam_density_ok_ln, auto.label = T, string.ci='95%CI', title = "Generalized additive model on log(sampling density)", dv.labels = c("Multiplied # of observations", "Actual # of observations")) #summary(glht(gam_density_ok_ln)) corrected p-values
#' 
...
#' <br>
#' 
#' Note, models with log-transformed sampling density fit the data better (see [Model assumptions](#Model_ass)). 
#' 
#' <br>
#' 
#' ***
...
#' 
#' 
#' Our replication supports the same qualitative trends but quantifies a larger A-D disparity, and demonstrates that robustness improves with model complexity and adequate random-effect specification (Fig. [S6](#F_S6)).
#'
#' ***
#'
#' <br>
...
#' ***
#' 
#' We then specified negative binomial mixed-effect models with number of observations as a response, log-transformed polygon area (km²) as an offset, and year (continuous) in interaction with HOLC grade (four-level factor) as predictors while controlling for non-independence of data points in the random effects. 
#' 
...
#' (2) Explicitly nested random intercerpts of state, city, HOLC grade and unique sampling polygon id.  
